...
for ((b=0;b<$BATCHES;b++)); do
  ...
  rm -f results/files_root_${b}.txt
    ...
    for ((p=0;p<$PIPELINES;p++)); do
      ...
      if [ ! -f $F ]; then HL_TARGET=host-new_autoscheduler HL_SEED=root PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 HL_NUM_THREADS=24 numactl --cpunodebind=0 make bench 2>&1 | grep -v "Nothing to be done"; fi
      ...
      for ((s=0;s<$SCHEDULES;s++)); do
        ...
        if [ ! -f $F ]; then HL_TARGET=host-new_autoscheduler HL_SEED=$s PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 HL_NUM_THREADS=24 numactl --cpunodebind=0 make bench 2>&1 | grep -v "Nothing to be done"; fi
