...
struct LoopNode : LoopLevelNode {
  ...
  LoopNode(Function f, int stage_index, int64_t extent, bool vectorized, const BlockNode* parent, int depth, bool parallel, TailStrategy tail_strategy, VarOrRVar var, bool unrolled, int product_of_outer_loops, int64_t bytes_loaded_from_external_producers, const std::set<std::string>& nested_producers);
