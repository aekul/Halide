...
int &LoopNestFeaturizer::op_bucket(PipelineFeatures::OpType op_type, Type scalar_type) {
...
PipelineFeatures::ScalarType LoopNestFeaturizer::classify_type(Type t) {
...
void LoopNestFeaturizer::visit(const Call *op) {
  ...
  if (op->call_type == Call::Halide) {
    ...
    if (op->name == func.name()) {
      j = visit_memory_access(op->type, op->args.at(0), PipelineFeatures::AccessType::LoadSelf);
      op_bucket(PipelineFeatures::OpType::SelfCall, op->type)++;
    } else {
      j = visit_memory_access(op->type, op->args.at(0), PipelineFeatures::AccessType::LoadFunc);
    ...
    // args should be flattened to a single element
    user_assert(op->args.size() == 1);
    j = visit_memory_access(op->type, op->args.at(0), PipelineFeatures::AccessType::LoadImage);
...
LoopNestFeaturizer::Jacobian LoopNestFeaturizer::visit_memory_access(Type t, Expr arg, PipelineFeatures::AccessType type) {
...
void LoopNestFeaturizer::visit_store_args(Type t, Expr arg) {
  ...
  store_jacobian = visit_memory_access(t, arg, PipelineFeatures::AccessType::Store);
}
