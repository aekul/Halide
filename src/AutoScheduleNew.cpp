...
struct PartialScheduleNode {
    ...
    void create_loop_nest(const FunctionDAG &dag,
        ...
        for (int i = children.size() - 1; i >= 0; i--) {
            children[i]->create_loop_nest(dag, params, this, indent_level, depth + 1, block, store_at_bounds, compute_bounds, strides, parallelism, num_cores);
        }
        ...
        if (innermost) {
            ...
            auto compute = make_unique<ComputeNode>(node->func, arg, values, block);
...
struct State {
    ...
    bool calculate_cost(const FunctionDAG &dag, const MachineParams &params,
        ...
        std::map<std::string, int> strides;
        std::map<std::string, double> parallelism;
        root.create_loop_nest(dag, params, nullptr, 0, 0, block.get(), store_at_bounds, compute_bounds, strides, parallelism, params.parallelism);
