...
struct State {
    ...
    void generate_children(const FunctionDAG &dag,
        ...
        int num_children = 0;
        {
            ...
            if (node->stages.size() == 1 && !node->is_output) {
                ...
                if (child->calculate_cost(dag, params, throughput_predictor, loop_nest)) {
    ...
    pair<map<const FunctionDAG::Node::Stage *, LoopNest::StageScheduleState>, LoopNest::ScheduleData> apply_schedule(const FunctionDAG &dag, const MachineParams &params, bool obtain_loop_nest_only = false) {
        ...
        for (const auto &n : dag.nodes) {
            src += "Func " + n.func.name() + " = get_pipeline().get_func(" + std::to_string(i) + ");\n";
            i--;
        }
...
std::string generate_schedules_new(const std::vector<Function> &outputs,
    ...
    if (json_path.empty()) {  // do not store json dump
      optimal->calculate_cost(dag, params, tp, loop_nest, true, nullptr);
    } else {
      optimal->calculate_cost(dag, params, tp, loop_nest, true, &jdata);
    }
...
void autoschedule_test() {
    ...
    if (1) {
        ...
        debug(0) << "** Optimal schedule:\n";
        LoopNestRoot loop_nest;
        optimal->calculate_cost(dag, params, tp, loop_nest, true);
        if (tp) tp->join();
        optimal->dump();
        debug(0) << '\n';
